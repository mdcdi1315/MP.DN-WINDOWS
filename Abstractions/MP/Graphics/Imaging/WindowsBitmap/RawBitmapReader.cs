...
namespace MP.Graphics.Imaging.WindowsBitmap
{
    ...
    public unsafe sealed class RawBitmapReader : IImage
    {
        ...
        private DataStream rds;
        private IMemoryHandle memory;
        ...
        public RawBitmapReader(DataStream strm) : this()
        {
            rds = strm ?? throw new ArgumentNullException(nameof(strm));
            ...
            baseidx = rds.Seek(info.Size - sizeof(BITMAPINFOHEADER), SeekDisplacement.Current);
            ReadColors();
        ...
        /// <exception cref="NotSupportedException">Cannot find an appropriate decoder for the given image.</exception>
        public static RawBitmapReader RawDecode(MP.IO.DataStream strm)
        {
            RawBitmapReader dec = new();
            dec.rds = strm ?? throw new ArgumentNullException(nameof(strm));
            ...
            dec.baseidx = dec.rds.Seek(dec.info.Size - sizeof(BITMAPINFOHEADER), SeekDisplacement.Current);
            dec.ReadColors();
        ...
        /// <exception cref="ArgumentException"><paramref name="strm"/> was not both readable and seekable.</exception>
        /// <exception cref="NotSupportedException">Cannot find an appropriate decoder for the given image.</exception>
        public static RawBitmapReader RawBitmapIconDecode(MP.IO.DataStream strm)
        {
            RawBitmapReader dec = new();
            dec.rds = strm;
            ArgumentNullException.ThrowIfNull(dec.rds);
            if (dec.rds.CanSeek == false) { throw new ArgumentException("Stream was unseekable.", nameof(strm)); }
            if (dec.rds.CanRead == false) { throw new ArgumentException("Stream was unreadable.", nameof(strm)); }
            dec.beforehdridx = dec.rds.Position;
            dec.info = dec.rds.ReadStructure<BITMAPINFOHEADER>();
        ...
        private void ReadColors()
        {
            ...
            rds.Seek(baseidx, SeekDisplacement.Begin);
        ...
        private void Read8BPPLessRGB()
        {
            ...
            memory = SystemInfo.CreateNativeMemory(info.Width * ht * 4);
        ...
        private void Read16BPP_RGB565()
        {
            ...
            rds.Seek(info.ColorTablesCount * sizeof(RGBQUAD) , SeekDisplacement.Current);
        ...
        private void ReadRLE8()
        {
            ...
            RGBQUAD[] colortable = new RGBQUAD[info.ColorTablesCount];
            // Read all the color tables from the bitmap.
            for (int I = 0; I < colortable.Length; I++)
            {
                colortable[I] = rds.ReadStructure<RGBQUAD>();
            }
            memory = SystemInfo.CreateNativeMemory(info.Height * info.Width * 4);
            memory.ZeroMemory(); // Zero the entire memory contents.
            
            RLEDecoder.ReadRLE8(memory.MemoryPointer , rds, info, colortable);
        }
        ...
        private void ReadRLE4()
        {
            ...
            RLEDecoder.ReadRLE4(memory.MemoryPointer , rds, info, colortable);
        }
        
        #endregion

        /// <summary>
        /// Specifies the image's horizontal resolution, if that information is available. <br />
        /// If not available, it will return 0.
        /// </summary>
        public System.Single HorizontalResolution => info.HorizontalResolution;

        /// <summary>
        /// Specifies the image's vertical resolution, if that information is available. <br />
        /// If not available, it will return 0.
        /// </summary>
        public System.Single VerticalResolution => info.VerticalResolution;

        /// <summary>
        /// Gets the number of bits that were used to store a single pixel. <br />
        /// This is only returned for reference how the original image was defined.
        /// </summary>
        public System.Int32 BitsPerPixel => info.BitCount;

        /// <summary>
        /// Gets the number of color tables used by the bitmap. <br />
        /// Only defined for images that have a <see cref="BitsPerPixel"/> value less than 8. <br />
        /// This is only returned for reference how the original image was defined.
        /// </summary>
        public System.UInt32 ColorTables => info.ColorTablesCount;

        /// <summary>
        /// Gets a value whether this bitmap image is effectively flipped vertically. <br />
        /// It can be unflipped by flipping it again by using the <see cref="IImageExtensions.FlipVertically(IImage)"/> method.
        /// </summary>
        public System.Boolean IsFlippedVertically => info.Height > 0;

        /// <summary>
        /// Gets the pointer to the decoded and raw bitmap image data.
        /// </summary>
        ...
        /// <summary>
        /// Gets the pixel data depiction in the bitmap image.
        /// </summary>
        public ImagePixelFormat PixelFormat => ImagePixelFormat.ARGB;

        /// <summary>
        /// Gets the size of this bitmap image.
        /// </summary>
        ...
        /// <summary>
        /// Disposes this <see cref="RawBitmapReader"/> class instance.
        /// </summary>
        public void Dispose() 
        {
            // Multithreading with native pointer management can make the best recipe for AVE's...
            // At least let's assure that only one thread will access this at a time, thus assuring that AVE's will not happen at least.
            try {
                Monitor.Enter(this);
                memory?.Dispose();
                memory = null;
