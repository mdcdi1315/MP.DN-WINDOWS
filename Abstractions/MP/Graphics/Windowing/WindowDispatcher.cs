...
namespace MP.Graphics.Windowing
{
    ...
    /// </summary>
    public sealed class WindowDispatcher : IDisposable, ISyncronized
    {
        ...
        private SingleLinkedListBasedQueue<DelegateData> delegates;
        private SingleLinkedList<DelegateData> delegatesineverytick;
        ...
        { 
            delegates = new SingleLinkedListBasedQueue<DelegateData>();
            delegatesineverytick = new SingleLinkedList<DelegateData>();
        ...
        public void RegisterAction(Action action)
        {
            Monitor.Enter(this);
            try {
                delegates.Enqueue(new DelegateData { Delegate = action, Arguments = null });
            } finally {
                Monitor.Exit(this);
            }
        }
            ...
            where TDLG : Delegate
        {
            Monitor.Enter(this);
            try {
                delegates.Enqueue(new DelegateData { Delegate = @delegate, Arguments = null });
            } finally {
                Monitor.Exit(this);
            }
        }
        ...
        /// </summary>
        /// <param name="action">The action to register</param>
        public void RegisterActionInEveryFrame(Action action)
        {
            Monitor.Enter(this);
            try {
                delegatesineverytick.Add(new DelegateData() { Delegate = action, Arguments = null });
            } finally {
                Monitor.Exit(this);
            }
        }
        ...
        public System.Boolean RemoveMethodInEveryFrame<TDLG>(TDLG @delegate)
            where TDLG : Delegate
        {
        ...
        /// <summary>
        /// Disposes this <see cref="WindowDispatcher"/> instance. <br />
        /// This is typically called by the <see cref="Window"/> that is using this, do not call this by yourself!
        /// </summary>
        public void Dispose()
        {
            Monitor.Enter(this);
            try {
                delegates.Clear();
                delegatesineverytick.Clear();
                delegates = null;
                delegatesineverytick = null;
