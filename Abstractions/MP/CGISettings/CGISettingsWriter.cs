...
namespace MP.CGISettings
{
    ...
    public sealed class CGISettingsWriter : ICGISettingsWriter<SettingEntry> , IStreamOwnerBase
    {
        ...
        private List<ICGISettingExtension> exts;
        ...
        public CGISettingsWriter(System.IO.Stream stream) : this(stream , true) { }
        ...
        /// <exception cref="ArgumentException"><paramref name="stream"/> was unwriteable.</exception>
        public CGISettingsWriter(System.IO.Stream stream, System.Text.Encoding encoding) : this(stream, true)
        {
            if (encoding is null)
            {
        ...
        /// <exception cref="ArgumentNullException"><paramref name="stream"/> was <see langword="null"/>.</exception>
        /// <exception cref="ArgumentException"><paramref name="stream"/> was unwriteable.</exception>
        public CGISettingsWriter(System.IO.Stream stream, System.Boolean leaveopen) : this()
        {
            if (stream is null)
            {
        ...
        private void WriteHeader() 
        {
            ...
            header.Padding = (Unsafe.SizeOf<CGIHEADER>() + header.AppNameLength) % 4;
        ...
        {
            if (stream is null) { throw new ObjectDisposedException(nameof(CGISettingsWriter)); }
        ...
        private CGISettingType GetCorrespondingTypeForObject(System.Object obj)
        {
            ...
            {
                if (exts[I].WrappingType == t)
                {
                    return exts[I].RegisteredType;
                }
            }
            t = null;
        ...
        private ConstructedSettingEntry GetArrayData(SettingEntry se)
        {
            ...
            System.Int32 elements = se.ValueList.Count;
            // elements * 2 due to the int's stored before the element.
            System.Byte[][] arrays = new System.Byte[elements * 2][];
            System.Int32 arraysfinali = 0;
            System.Int64 finalarraylen = 0;
            for (System.Int32 I = 0; I < elements; I++)
            {
                // Construct a temporary and forged SettingEntry to use the GetData method.
                // Otherwise we will only retrieved the constructed data and nothing else.
                var eldata = GetData(new() {
                    Name = "___EDMMMMM",
                    Value = se.ValueList[I],
                    IsArrayOfType = false,
                    Type = type
                });
                arrays[arraysfinali] = eldata.Data.Length.GetBytes();
                if (arrays[arraysfinali].Length > System.Array.MaxLength - 4)
                {
                    throw new InvalidOperationException("Attempted to add an object which surpasses the defined CGI setting size limits.");
                }
                finalarraylen += arrays[arraysfinali].Length;
                arraysfinali++;
                arrays[arraysfinali] = eldata.Data;
                eldata = null;
                finalarraylen += arrays[arraysfinali].Length;
                arraysfinali++;
            }
            System.Byte[] result = new System.Byte[finalarraylen];
            System.Int64 resultindex = 0;
            for (System.Int32 I = 0; I < arrays.Length; I++)
            {
                System.UInt32 ct = arrays[I].Length.ToUInt32();
                arrays[I].Copy(0 , result , resultindex , ct);
                arrays[I] = null; // Directly delete the array so that it is freed
                resultindex += ct; // And update the index by the number of copied bytes...
            }
            // Delete the jagged array variable.
            arrays = null;
            return new(result, type) { IsArrayType = true , NumberOfElements = elements.ToUInt16() };
        }
        ...
        public void Generate()
        {
            ...
            foreach (var setting in settings)
            {
                ...
                settinghdr.CoreHeader.Length = value.Length;
