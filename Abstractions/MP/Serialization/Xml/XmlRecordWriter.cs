...
namespace MP.Serialization.Xml
{
    ...
    public sealed class XmlRecordWriter : IRecordWriter
    {
        ...
        private static void AddArrayElement(System.Object value , XmlWriter writer)
        {
            ...
            AddValueDirect(value, writer);

            writer.WriteEndElement();
        }

        private static void AddValueDirect(System.Object value, XmlWriter writer)
        {
            switch (value)
            {
        ...
        private static void AddRecord(Record rc , XmlWriter writer)
        {
            ...
            {
                sft = i.Type;
                if (sft.HasFlag(SerializedFieldType.Array) || sft.HasFlag(SerializedFieldType.List))
                {
                    writer.WriteStartElement(XmlSerializationConstants.SerializedElementTagName);
                    ...
                    if (sft.HasFlag(SerializedFieldType.Array)) {
                        ...
                        writer.WriteAttributeString(XmlSerializationConstants.ElementTypeAttributeName, XmlSerializationConstants.GetSimpleXmlFieldType(sft & ~SerializedFieldType.List));
                        writer.WriteAttributeString(XmlSerializationConstants.ElementIsListAttributeName, "true");
                    }
                    foreach (System.Object o in (System.Collections.IEnumerable)i.Value) { AddArrayElement(o, writer); }
                    writer.WriteEndElement();
                } else if (sft.HasFlag(SerializedFieldType.StrictStringDictionary)) {
                    writer.WriteStartElement(XmlSerializationConstants.SerializedElementTagName);
                    writer.WriteAttributeString(XmlSerializationConstants.ElementNameAttributeName, i.Name);
                    writer.WriteAttributeString(XmlSerializationConstants.ElementTypeAttributeName, XmlSerializationConstants.GetSimpleXmlFieldType(sft & ~SerializedFieldType.StrictStringDictionary));
                    writer.WriteAttributeString(XmlSerializationConstants.ElementIsStringDictionaryAttributeName, "true");
                    foreach (Record r in i.RecordArrayValue)
                    {
                        writer.WriteStartElement("entry");
                        writer.WriteAttributeString("name", r.LookupField("key").Value as string);
                        AddValueDirect(r.LookupField("value").Value, writer);
                        writer.WriteEndElement();
                    }
        ...
        public void WriteNew(DataStream stream, Record record)
        {
            ...
            try {
                writer = XmlWriter.Create(new DataStreamToStreamAdapter(stream), new() { CloseOutput = false, ConformanceLevel = ConformanceLevel.Document, Indent = true, NewLineOnAttributes = false });
