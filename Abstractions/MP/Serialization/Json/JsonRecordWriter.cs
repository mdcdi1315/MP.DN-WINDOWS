...
namespace MP.Serialization.Json
{
    ...
    public sealed class JsonRecordWriter : IRecordWriter
    {
        ...
        public void WriteNew(Stream stream, Record record)
        {
            ...
            writer = new(stream, new JsonWriterOptions() { Indented = true, SkipValidation = true, MaxDepth = 50 });
            try {
        ...
        private static void WriteRecord(Utf8JsonWriter writer , Record rc)
        {
            ...
            {
                if (fd.Type.HasFlag(SerializedFieldType.Array)) {

                    sft = fd.Type & ~SerializedFieldType.Array;
                    ...
                    if (rcs is not null) 
                    {
                        ...
                        foreach (Object value in (fd.Value as Array))
                        {
                            switch (sft)
                            {
