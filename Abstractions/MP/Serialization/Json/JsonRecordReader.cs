    ...
    public sealed class JsonRecordReader : IRecordReader
    {
            ...
            foreach (var element in root.EnumerateObject())
            {
                switch (element.Value.ValueKind)
                {
                    case JsonValueKind.Array:
                        builder.Add(new SerializedField(element.Name , DecodeArray(element.Value)));
                        break;
        ...
        private static Array DecodeArray(JsonElement sourcearray)
        {
            ...
            try {
                for (int I = 0; I < al; I++)
                {
                    target.SetValue(DecodeTypeAsObject(sourcearray[I]), I);
                }
        ...
        public void InitializeForNewPayload(DataStream stream)
        {
            ...
            doc = JsonDocument.Parse(new DataStreamToStreamAdapter(stream), new JsonDocumentOptions() { CommentHandling = JsonCommentHandling.Skip, MaxDepth = 50 });
        }
