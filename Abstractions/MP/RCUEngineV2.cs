...
namespace MP
{
    ...
    /// -&gt; It is called 'Runtime Compiled Units' since the engine must be aware for all compiled IL command sequences (through the provided delegate signatures)
    /// that should be run under a specific engine state.
    /// </summary>
    /// <remarks>
    /// Some changes were done in V2 to allow better type safety of RCU Engine objects. <br />
    /// Some of those changes removed some freedom on how the user can manage the engine and what can do with it to assure higher stability expectations.
    /// </remarks>
    public abstract class RCUEngineV2 : IAttributeable, IDisposable
    {
        private IOperationsTasker tasker;
        private HashSet<SendCommandDelegate> delegates;
        ...
        public RCUEngineV2()
        {
            ...
            delegates = new(2, new DelegateEqualityComparer<SendCommandDelegate>());
        ...
        public System.Boolean TryGetAttribute(System.String attribute, out System.Object value)
        {
            ArgumentNullException.ThrowIfNullOrEmpty(attribute);
        ...
        private void BroadcastSendCommand(SendCommandDataEventArgs e)
        {
            ...
            Monitor.Enter(delegates);
            IEnumerator<SendCommandDelegate> scd = delegates.GetEnumerator();
            try {
                while (scd.MoveNext()) {
                    scd.Current.Invoke(this, e);
                }
            } finally {
                Monitor.Exit(delegates); // Release access after done with broadcasting
                scd.Dispose(); // Our enumerator can be disposed of at any time...
            }
        }
        ...
        /// </summary>
        /// <exception cref="ObjectDisposedException">The engine was disposed.</exception>
        public IEnumerable<OperationsTaskerWorkItemFailureData> FailedExecutables
        {
            [Throws(typeof(ObjectDisposedException))]
            get => (tasker ?? throw new ObjectDisposedException(GetType().FullName)).FailedWorkItems;
        }
        ...
        /// <exception cref="ObjectDisposedException">The engine was disposed.</exception>
        [Throws(typeof(ObjectDisposedException))]
        protected void WaitUntilCurrentOperationCompletion()
        {
            ObjectDisposedException.ThrowIf(tasker is null, this);
            tasker.JoinCurrentOperation();
        }
