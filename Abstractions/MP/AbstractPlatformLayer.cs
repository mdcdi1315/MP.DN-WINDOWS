...
namespace MP
{
    ...
    {
        /// <summary>
        /// Provides the string value for the default memory manager registration. <br />
        /// This is a constant value.
        /// </summary>
        public const String DefaultMemoryManager = "DEFAULT";

        private Dictionary<String, INativeMemoryManager> mem_managers;

        /// <summary>
        /// Initializes a new instance of the <see cref="AbstractPlatformLayer"/> class.
        /// </summary>
        public AbstractPlatformLayer() {
            mem_managers = new();
        }

        internal void UnloadLayerImpl()
        {
            if (mem_managers is not null)
            {
                mem_managers.Clear();
                mem_managers = null;
            }
        ...
        /// </summary>
        protected virtual void UnloadLayer() { }

        /// <summary>Gets a previously registered memory manager.</summary>
        /// <param name="name">The identifier of the native memory manager to get.</param>
        /// <returns>The native memory manager corresponding to <paramref name="name"/>.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="name"/> is <see langword="null"/>.</exception>
        /// <exception cref="KeyNotFoundException"><paramref name="name"/> is not a valid native memory manager name.</exception>
        [Throws(typeof(ArgumentNullException), typeof(KeyNotFoundException))]
        public INativeMemoryManager GetMemoryManager(String name) => mem_managers[name];

        /// <summary>Gets the default memory manager for this platform.</summary>
        /// <returns>The default memory manager.</returns>
        [Throws(typeof(KeyNotFoundException))]
        public INativeMemoryManager GetDefaultMemoryManager() => mem_managers[DefaultMemoryManager];

        /// <summary>
        /// Registers a new memory manager to this instance.
        /// </summary>
        /// <param name="name">The name of the memory manager.</param>
        /// <param name="manager">The native memory manager instance to associate with <paramref name="name"/>.</param>
        /// <exception cref="ArgumentNullException"><paramref name="name"/> is <see langword="null"/>.</exception>
        /// <exception cref="ArgumentException">A native memory manager is already registered with the specified <paramref name="name"/>.</exception>
        [Throws(typeof(ArgumentNullException), typeof(ArgumentException))]
        public void RegisterMemoryManager(String name, INativeMemoryManager manager)
        {
            ArgumentNullException.ThrowIfNull(manager);
            mem_managers.Add(name, manager);
        }
