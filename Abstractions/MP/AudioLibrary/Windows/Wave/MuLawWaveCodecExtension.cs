...
{
    /// <summary>
    /// Wave file codec extension for Mu-Law data.
    /// </summary>
    public sealed class MuLawWaveCodecExtension : WaveCodecExtension
    {
        private bool is_ok;
        private AudioFormat format;

        /// <summary>Initializes a new instance of the <see cref="MuLawWaveCodecExtension"/> class.</summary>
        /// <param name="stream">The data stream passed through the <see cref="WaveFileAudioCodec"/> class.</param>
        public MuLawWaveCodecExtension(DataStream stream) : base(stream) { }
        ...
        protected unsafe override void ProcessChunk(ChunkInfo ch)
        {
            if (ch.ID == "fmt ") {
                if (ch.Length == sizeof(WAVEFORMATEXTENSIBLE)) {
                    ...
                    if (ext.SubFormatGUID.ToString() == "00000007-0000-0010-8000-00aa00389b71") {
        ...
        public override int Read(Span<byte> buffer)
        {
            ...
            if (read > 0) {
                ...
                while (I < read)
                {
                    buffer.WriteStructure(G, MuLawDecoder.MuLawToLinearSample(temp[I++]));
                    G += sizeof(short);
                }
