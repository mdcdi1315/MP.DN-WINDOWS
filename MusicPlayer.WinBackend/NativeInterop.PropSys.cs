...
partial class Interop
{
    ...
    public unsafe static class PropSys
    {
        ...
        private static extern HRESULT PSCreateMemoryPropertyStore_Native(GUID* iid, void** cobj);

        [DllImport(Libraries.PropSys, EntryPoint = "InitPropVariantFromCLSID", ExactSpelling = true)]
        private static extern HRESULT InitPropVariantFromCLSID_Native(GUID* clsid, PROPVARIANT* propvariant);
        ...
        {
            HRESULT hrt;
            fixed (System.Char* pkeystr = (keystr = new('\0', PKEYSTR_MAX + 2))) {
                hrt = PSStringFromPropertyKey_Native(&key, pkeystr, keystr.Length.ToUInt32());
            }
            System.Int32 s0idx;
            if (hrt.SUCCEEDED && (s0idx = keystr.IndexOf('\0')) > -1) { keystr = keystr.Remove(s0idx); }
            return hrt;
        }
        ...
        public static HRESULT PSGetNameFromPropertyKey(PROPERTYKEY key , out System.String name)
        {
            ...
            {
                name = new(poutname);
                WindowsCOMLibrary.COMMemoryManager.Free(poutname);
            }
            return hrt;
        }
       
        public static HRESULT PSCreateMemoryPropertyStore(GUID iid , out void* pinterface)
        {
            void* pcobj;
        ...
        public static HRESULT InitPropVariantFromCLSID(GUID guid, out PROPVARIANT pv)
        {
            PROPVARIANT pvt;
