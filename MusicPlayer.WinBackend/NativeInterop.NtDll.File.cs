...
unsafe partial class Interop
{
    public static partial class NtDll
    {
        ...
        public struct FILE_FULL_DIR_INFORMATION
        {
            ...
            /// </remarks>
            [FieldOffset(56)]
            public MP.IO.FileAttributes FileAttributes;
        ...
        private struct FILE_RENAME_INFORMATION_OLD
        {
            ...
            public static IMemoryHandle GetRenameInformation(System.Boolean replaceifexists , System.String name, System.IntPtr root)
            {
                System.UInt32 leninbytes = name.Length.ToUInt32() * sizeof(System.Char);
                IMemoryHandle mem = SystemInfo.GetDefaultMemoryManager().AllocateMemoryHandle((sizeof(FILE_RENAME_INFORMATION_OLD) + leninbytes).ToUInt64());
        ...
        private struct FILE_RENAME_INFORMATION_NEW
        {
            ...
            public static IMemoryHandle GetRenameInformation(System.Boolean replaceifexists, System.String name, System.IntPtr root)
            {
                ...
                IMemoryHandle mem = SystemInfo.GetDefaultMemoryManager().AllocateMemoryHandle((sizeof(FILE_RENAME_INFORMATION_NEW) + leninbytes).ToUInt64());
        ...
        /// <summary>
        /// Flag definitions for NtFlushBuffersFileEx <br /> <br />
        /// 
        /// If none of the below flags are specified the following will occur for a given file handle: <br />
        /// <list type="bullet">
        ///     <item>Write any modified data for the given file from the Windows in-memory cache.</item>
        ///     <item>Commit all pending metadata changes for the given file from the Windows in-memory cache.</item>
        ///     <item>Send a SYNC command to the underlying storage device to commit all written data in the devices cache to persistent storage.</item>
        /// </list>
        /// <br /> <br />
        /// If a volume handle is specified: <br />
        /// <list type="bullet">
        ///     <item>Write all modified data for all files on the volume from the Windows in-memory cache.</item>
        ///     <item>Commit all pending metadata changes for all files on the volume from the Windows in-memory cache.</item>
        ///     <item>Send a SYNC command to the underlying storage device to commit all written data in the devices cache to persistent storage.</item>
        /// </list>
        /// <br /> <br />
        ...
        /// </summary>
        [Flags]
        public enum NtFlushFileBuffersFlags : System.UInt32
        {
            /// <summary>
            /// File data and metadata in the file cache will be written, and the underlying storage is synchronized to flush its cache. <br />
            /// Windows file systems supported: NTFS, ReFS, FAT, exFAT.
            /// </summary>
            Default = 0,
            /// <summary>
            /// If set, this operation will write the data for the given file from the Windows in-memory cache.  
            /// This will NOT commit any associated metadata changes.  
            /// This will NOT send a SYNC to the storage device to flush its cache. <br />
            /// Not supported on volume handles.  <br />
            /// Windows file systems supported: NTFS, FAT, exFAT.
            /// </summary>
            FLUSH_FLAGS_FILE_DATA_ONLY = 0x00000001,
            /// <summary>
            /// If set, this operation will commit both the data and metadata changes for the given file from the Windows in-memory cache.  
            /// This will NOT send a SYNC to the storage device to flush its cache. <br />
            /// Not supported on volume handles. <br />
            /// Windows file systems supported: NTFS, FAT, exFAT.
            /// </summary>
            ...
            /// <summary>
            /// If set, this operation will write the data for the given file from the Windows in-memory cache.  
            /// It will also try to skip updating the timestamp as much as possible. 
            /// This will send a SYNC to the storage device to flush its cache. <br />
            /// Not supported on volume or directory handles. <br />
        ...
        [DllImport(Libraries.NtDll, EntryPoint = "NtCreateFile", ExactSpelling = true)]
        private static extern NTSTATUS NtCreateFile_Native(
            IntPtr* FileHandle,
        ...
        public sealed class CreateFileInfo
        {
        ...
        [DllImport(Libraries.NtDll , EntryPoint = "NtQueryDirectoryFile", ExactSpelling = true)]
        ...
        [DllImport(Libraries.NtDll , EntryPoint = "NtSetInformationFile", ExactSpelling = true)]
        ...
        [DllImport(Libraries.NtDll, EntryPoint = "NtReadFile", ExactSpelling = true)]
        private static extern NTSTATUS NtReadFile_Native(
            ...
            System.UInt32* Key = null // Unused, set this to NULL
        );

        [DllImport(Libraries.NtDll, EntryPoint = "NtWriteFile", ExactSpelling = true)]
        ...
        [DllImport(Libraries.NtDll, EntryPoint = "NtFlushBuffersFileEx", ExactSpelling = true)]
        private static extern NTSTATUS NtFlushBuffersFileEx_Native(
            IntPtr file_handle,
            NtFlushFileBuffersFlags flags,
            void* parameters,
            uint parameters_size,
            IO_STATUS_BLOCK* p_block
        );

        [DllImport(Libraries.NtDll, EntryPoint = "NtFlushBuffersFile", ExactSpelling = true)]
        private static extern NTSTATUS NtFlushBuffersFile_Native(
            IntPtr file_handle,
            IO_STATUS_BLOCK* p_block
        );

        [DllImport(Libraries.NtDll, EntryPoint = "NtDeleteFile", ExactSpelling = true)]
        private static extern NTSTATUS NtDeleteFile_Native(OBJECT_ATTRIBUTES* p_attributes);

        // NOTE: If you want to access a fully qualified DOS path using this function, prefix your path string with \??\ .
        [RequiresNativeLayer]
        public static CreateFileInfo NtCreateFile(
            ...
            uint eaLength = 0)
        {
            ...
            var status = NtCreateFile_Native(
                &handle,
                desiredAccess,
                &attributes,
                &sbb,
        ...
        public static NTSTATUS NtDeleteFile(ReadOnlySpan<System.Char> path, IntPtr root_dir)
        {
            UNICODE_STRING str = UNICODE_STRING.CreateFromSpanExact(path);
            OBJECT_ATTRIBUTES attributes = new(&str, ObjectAttributes.OBJ_CASE_INSENSITIVE, root_dir);
            return NtDeleteFile_Native(&attributes);
        }

        public static NTSTATUS NtFlushFileBuffers(System.IntPtr hfe, out IO_STATUS_BLOCK block)
        {
            NTSTATUS nts;
            IO_STATUS_BLOCK io;
            nts = NtFlushBuffersFile_Native(hfe, &io);
            block = io;
            return nts;
        }

        public static NTSTATUS NtFlushFileBuffersEx(System.IntPtr hfe, NtFlushFileBuffersFlags flags, out IO_STATUS_BLOCK block)
        {
            NTSTATUS nts;
            IO_STATUS_BLOCK io;
            try {
                nts = NtFlushBuffersFileEx_Native(hfe, flags, null, 0U, &io);
            } catch (EntryPointNotFoundException) {
                // If not existing, delegate to the NtFlushFileBuffers call. (Windows 7)
                nts = NtFlushBuffersFile_Native(hfe, &io);
            }
            block = io;
            return nts;
        ...
        {
            IO_STATUS_BLOCK blk;
            System.UInt32 allocated_len = 504U;
            FILE_NAME_INFORMATION* pf = (FILE_NAME_INFORMATION*)SystemInfo.GetDefaultMemoryManager().Allocate(allocated_len);
            pf->FileNameLength = allocated_len;
            NTSTATUS nts;

        G_retry:
            nts = NtQueryInformationFile_Native(hfe, &blk, pf, allocated_len, FILE_INFORMATION_CLASS.FileNameInformation);

            switch (nts)
            {
                case NTSTATUS.STATUS_SUCCESS:
                    filename = FILE_NAME_INFORMATION.ReadName(pf);
                    SystemInfo.GetDefaultMemoryManager().Free(pf);
                    return NTSTATUS.STATUS_SUCCESS;
                case NTSTATUS.STATUS_BUFFER_OVERFLOW:
                    allocated_len += 500U;
                    pf = (FILE_NAME_INFORMATION*)SystemInfo.GetDefaultMemoryManager().ReAllocate(pf, allocated_len);
                    pf->FileNameLength = allocated_len;
                    goto G_retry;
                default:
                    SystemInfo.GetDefaultMemoryManager().Free(pf);
        ...
        public static NTSTATUS NtSetInformationFile(System.IntPtr hfe, FILE_DISPOSITION_INFORMATION disposition, out IO_STATUS_BLOCK stat)
        {
            IO_STATUS_BLOCK blk;
            NTSTATUS nts = NtSetInformationFile_Native(hfe, &blk, &disposition, sizeof(FILE_DISPOSITION_INFORMATION).ToUInt32(), FILE_INFORMATION_CLASS.FileDispositionInformation);
            stat = blk;
            return nts;
        }
        ...
        public static NTSTATUS NtSetInformationFile(System.IntPtr hfe, FILE_ACCESS_INFORMATION f, out IO_STATUS_BLOCK stat)
        {
            IO_STATUS_BLOCK blk;
            NTSTATUS s = NtSetInformationFile_Native(hfe, &blk, &f, sizeof(FILE_ACCESS_INFORMATION).ToUInt32(), FILE_INFORMATION_CLASS.FileAccessInformation);
            stat = blk;
            return s;
        }
