...
namespace MP.NativeInterop.Windows.COM
{
    ...
    public unsafe sealed class PropertyStore : IDisposable
    {
        ...
        public static PropertyStore LoadFrom(MP.IO.DataStream stream)
        {
            ...
            byte[] buftemp = stream.ReadBytes(stream.Length);
            // Now load the COM object
            HRESULT hr = Interop.PropSys.PSCreateMemoryPropertyStore(GUID.FromString(CommonInteropClsIds.IID_IPersistSerializedPropStorage), out void* pi);
            hr.ThrowOnFailure();
            IPersistSerializedPropStorage storage = ComInterop.CreateRCW<IPersistSerializedPropStorage>(pi);
            // Provide the data to the deserializer
            fixed (byte* pdt = buftemp)
            {
                hr = storage.SetPropertyStorage(pdt, buftemp.Length.ToUInt32());
            }
            hr.ThrowOnFailure();
            // return the property store by using QueryInterface on it
            return new(ComInterop.QueryInterface<IPropertyStore>(storage));
        }
        ...
        {
            Interop.PropSys.PSCreateMemoryPropertyStore(GUID.FromString(CommonInteropClsIds.IID_IPropertyStore) , out void* pi).ThrowOnFailure();
            return new(ComInterop.CreateRCW<IPropertyStore>(pi));
        }
        ...
        /// <summary>Gets a property in it's entirety.</summary>
        ...
        /// <exception cref="ObjectDisposedException">The object was disconnected from the native COM object.</exception>
        public bool TryGetProperty(PROPERTYKEY key , out PropertyStoreProperty property)
        {
            ObjectDisposedException.ThrowIf(store is null, this);
            PROPVARIANT vt;
        ...
        /// <exception cref="ObjectDisposedException">The object was disconnected from the native COM object.</exception>
        public void Save(MP.IO.DataStream stream)
        {
            ...
            IPersistSerializedPropStorage storage =
                ComInterop.QueryInterface<IPersistSerializedPropStorage>(store) ?? 
            ...
            try {
                uint blks = datalen / BUFSIZE, rem = datalen % BUFSIZE;
                uint idx = 0;
                ...
                WindowsCOMLibrary.COMMemoryManager.Free(pbuf);
            }
        }
    ...
    public sealed class PropertyStoreProperty
    {
        ...
        public object Value => propertynative.Value;
        ...
        unsafe ~PropertyStoreProperty()
        {
            ...
            fixed (PROPVARIANT* pv = &propertynative)
            {
                ...
                DebugProvider.WriteLine($"ObjectGraphTracker: Freed PROPVARIANT {new nint(pv)} and error code was {hrt}.");
#else
