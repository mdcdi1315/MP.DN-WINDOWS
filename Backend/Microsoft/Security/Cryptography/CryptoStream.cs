...
namespace Microsoft.Security.Cryptography
{
    public class CryptoStream : Stream, IDisposable
    {
                ...
                else
                {
                    ...
                    fixed (byte* p = &rentedBuffer[0])
                    {
                        try
                        {
                            ...
                            Unsafe.InitBlockUnaligned(p, 0, (System.UInt32)rentedBuffer.Length);
                        }
                    }
        ...
        private static SemaphoreSlim SemaphoreActivateFunction() => new(1 , 1);
        ...
        private SemaphoreSlim AsyncActiveSemaphore
        {
            get
            {
                ...
                return LazyInitializer.EnsureInitialized(ref _lazyAsyncActiveSemaphore, new(SemaphoreActivateFunction));
            }
        }
    }
}
