...
namespace Microsoft.Security.Cryptography
{
    ...
    internal sealed class HMACCommon
    {
        ...
        private byte[]? ChangeKeyImpl(ReadOnlySpan<byte> key)
        {
            ...
            if (key.Length > _blockSize && _blockSize > 0)
            {
                ...
                switch (_hashAlgorithmId)
                {
                    ...
                    case HashAlgorithmNames.MD5:
                        ...
                        break;
                    default:
                        throw new CryptographicException(System.String.Format(SR.Cryptography_UnknownHashAlgorithm, _hashAlgorithmId));
                };
            }
            ...
            _hMacProvider = new BCryptHashProvider(_hashAlgorithmId, true, key.ToArray());
