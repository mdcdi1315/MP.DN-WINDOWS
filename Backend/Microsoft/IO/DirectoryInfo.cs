...
namespace Microsoft.IO
{
    public sealed partial class DirectoryInfo : FileSystemInfo
    {
        ...
        public DirectoryInfo CreateSubdirectory(string path)
        {
            ...
            // We weren't nested
            throw new ArgumentException(System.String.Format(SR.Argument_InvalidSubPath, path, FullPath), nameof(path));
        }
        ...
        public void MoveTo(string destDirName)
        {
            ...
            if (!Exists && !FileSystem.FileExists(FullPath))
                throw new DirectoryNotFoundException(System.String.Format(SR.IO_PathNotFound_Path, FullPath));

            if (FileSystem.DirectoryExists(destination))
                throw new IOException(System.String.Format(SR.IO_AlreadyExists_Name, destinationWithSeparator));
