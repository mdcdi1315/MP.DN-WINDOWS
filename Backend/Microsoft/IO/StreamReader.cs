...
namespace Microsoft.IO
{
    ...
    public class StreamReader : TextReader
    {
        ...
        private static FileStream ValidateArgsAndOpenPath(string path, Encoding encoding, FileStreamOptions options)
        {
            ...
            if ((options.Access & System.IO.FileAccess.Read) == 0)
            {
                throw new ArgumentException(SR.Argument_StreamNotReadable, nameof(options));
            }

            return new FileStream(path, (FileMode)options.Mode , (FileAccess)options.Access , (FileShare)options.Share);
        }
        ...
        public override Task<System.String> ReadLineAsync() =>
        ...
        internal virtual async ValueTask<int> ReadAsyncInternal(Memory<char> buffer, CancellationToken cancellationToken)
        {
            if (_charPos == _charLen && (await ReadBufferAsync(cancellationToken).ConfigureAwait(false)) == 0)
            {
                return 0;
