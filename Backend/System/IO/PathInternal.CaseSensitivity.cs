...
namespace System.IO
{
    ...
    partial class PathInternal
    {
        ...
        private static bool GetIsCaseSensitive()
        {
            try
            {
                string pathWithUpperCase = Path.Combine(Microsoft.IO.Path.GetTempPath(), "CASESENSITIVETEST" + Guid.NewGuid().ToString("N"));
                // Opening a file handle to test for case sensitivity is more than enough.
                using (Microsoft.Win32.SafeHandles.RedistSafeFileHandle.Open(pathWithUpperCase , Microsoft.IO.FileMode.CreateNew , Microsoft.IO.FileAccess.ReadWrite , Microsoft.IO.FileShare.None , Microsoft.IO.FileOptions.DeleteOnClose , 0x1000))
                {
                    string lowerCased = pathWithUpperCase.ToLowerInvariant();
                    return !Microsoft.IO.File.Exists(lowerCased);
                }
            } catch (Exception exc) {
