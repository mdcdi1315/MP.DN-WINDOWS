...
{
    /// <summary>
    /// Reads native M4A tags on full .NET . Pass it a stream with a M4A file , and it will read the tag if there is one.
    /// </summary>
    public sealed class MP4AudioTagReader : ITagReader2
    {
        ...
        private void ReadData()
        {
            ...
            Microsoft.IO.MemoryStream mem = null;
            try {
                Box g = GetAtom("moov");
        ...
        {
            Microsoft.IO.MemoryStream mem = new(parent.Data);
            mem.Position = 4;
            Box d;
