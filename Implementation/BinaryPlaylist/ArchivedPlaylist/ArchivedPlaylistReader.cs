...
{
    public sealed class ArchivedPlaylistReader : IDisposable
    {
        private BinaryPlaylistReader reader;
        private Dictionary<BlobTypes, PlaylistBlobReader> readerspooled;

        public ArchivedPlaylistReader(System.IO.Stream stream)
        {
            reader = new BinaryPlaylistReader(stream);
            readerspooled = new((System.Int32)BlobTypes.MAXEMBEDDEDBLOBVAL);
        }
        ...
        public ArchivedPlaylistAttributeBlobReader AttributeBlobReader => GetReader<ArchivedPlaylistAttributeBlobReader>(BlobTypes.MAXEMBEDDEDBLOBVAL + 3);

        public ArchivedPlaylistImageBlobReader ImageReader => GetReader<ArchivedPlaylistImageBlobReader>(BlobTypes.MAXEMBEDDEDBLOBVAL + 4);
